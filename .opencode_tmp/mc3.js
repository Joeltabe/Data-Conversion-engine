...
let mcSel = new Map();  // old matricule -> portal matricule (bulk migration selection)
...
function mcKeyOf(r){
  return (r.matricule || '') ? r.matricule : ('~' + (r.name || ''));
}

function mcSelToggle(key, newMat, name, on){
  if(on && newMat) mcSel.set(key, {new: newMat, name});
  else mcSel.delete(key);
  renderMatriculeResults(mcResults);
}

function mcManualMat(key, name, el){
  const v = el.value.trim().toUpperCase();
  const row = el.closest('.mrow');
  if(v) mcSel.set(key, {new: v, name});
  else mcSel.delete(key);
  if(row) row.querySelector('input[type=checkbox]').checked = !!v;
  renderMcBulkBar();
}

function mcManualToggle(key, name, cb){
  const row = cb.closest('.mrow');
  const inp = row ? row.querySelector('input[type=text]') : null;
  const v = inp ? inp.value.trim().toUpperCase() : '';
  if(cb.checked){
    if(!v){ toast('Enter a matricule first','warning'); cb.checked = false; return; }
    mcSel.set(key, {new: v, name});
  } else {
    mcSel.delete(key);
  }
  renderMcBulkBar();
...
function setMcSelAll(which){
  for(const r of (mcResults || [])){
    const key = mcKeyOf(r);
    const ok = which === 'all' || which === r.status;
    if(ok && r.api_matricule) mcSel.set(key, {new: r.api_matricule, name: r.name});
    else mcSel.delete(key);
  }
  renderMatriculeResults(mcResults);
}

function renderMcBulkBar(){
  const host = document.getElementById('mc-bulk-bar');
  if(!host) return;
  const n = mcSel.size;
  host.innerHTML = `<div class="flex gap-2 aic" style="margin:0 0 .75rem;padding:.5rem .75rem;background:var(--g1);border:1px solid var(--g2);border-radius:8px;flex-wrap:wrap">
    <span class="text-sm muted">Bulk migrate:</span>
    <button class="btn btn-outline btn-sm" onclick="setMcSelAll('mismatch')">Select mismatches</button>
    <button class="btn btn-outline btn-sm" onclick="setMcSelAll('review')">Select reviews</button>
    <button class="btn btn-outline btn-sm" onclick="setMcSelAll('all')">Select all matched</button>
    ${n?`<button class="btn btn-primary btn-sm" onclick="bulkMigrate()">ðŸšš Migrate ${n} â†’ portal</button>`:''}
    ${n?`<button class="btn btn-outline btn-sm" onclick="setMcSelAll('none')">Clear</button>`:''}
    <span class="text-sm muted">${n} selected</span>
  </div>`;
}

function manualRow(r){
  const key = mcKeyOf(r);
  const cur = mcSel.get(key);
  return `<div class="mrow" style="font-size:12px;padding:.3rem .5rem;background:var(--g0);border-radius:4px;display:flex;gap:8px;align-items:center;flex-wrap:wrap">
    <input type="checkbox" style="accent-color:var(--blue);width:14px;height:14px"
      ${cur?'checked':''} onchange="mcManualToggle('${esc(key)}','${esc(r.name)}',this)">
    <span style="color:var(--g5)">${esc(r.name||'(no name)')}</span>
    <span class="mono" style="color:var(--g6)">${r.matricule?esc(r.matricule):'(no matricule)'}</span>
    <input type="text" placeholder="New matriculeâ€¦" value="${esc(cur?cur.new:'')}"
      style="width:190px;font-size:12px;padding:.25rem .45rem;border:1px solid var(--g3);border-radius:6px;font-family:monospace"
      oninput="mcManualMat('${esc(key)}','${esc(r.name)}',this)">
    ${cur?`<button class="btn btn-sm btn-primary" onclick="bulkMigrate('${esc(key)}')">Migrate</button>`:''}
  </div>`;
}

async function bulkMigrate(onlyKey){
  if(!jobId) return;
  const targets = onlyKey
    ? new Map([[onlyKey, mcSel.get(onlyKey)]])
    : new Map(mcSel);
  if(!targets.size) return;
  const n = targets.size;
  if(!confirm(`Migrate ${n} student(s) to the matricule you provided?\n\nThis updates their matricule for the Excel export.`)) return;
  const mappings = [...targets.entries()].map(([key, v]) => ({
    old_matricule: key.startsWith('~') ? '' : key,
    old_name: v.name,
    new_matricule: v.new
  }));
  try{
    const r = await fetch(`/api/job/${jobId}/matricule-bulk`,{
    ...
    if(d.failures && d.failures.length)
      toast(`${d.failures.length} failed: ${d.failures[0].error}`,'warning');
    toast(`Migrated ${d.applied} of ${n} matricule(s)`,'success');
    for(const [key, v] of [...targets]){
      const x = mcResults.find(rr => mcKeyOf(rr) === key);
      if(x){ x.matricule = v.new; x.status = 'verified'; x.mismatch = false; }
      mcSel.delete(key);
    }
    await loadStudents();
    renderMatriculeResults(mcResults);
  }catch(e){ toast('Bulk migration failed: '+e.message,'error'); }
}
...
function renderMatriculeResults(results){
  ...
  const manualTargets = [...notfound, ...skipped].filter(r => r.matricule || r.name);
  if(mismatched.length || review.length || manualTargets.length)
    html += `<div id="mc-bulk-bar"></div>`;
  ...
  if(notfound.length){
    html += `<div style="margin-top:.5rem">
      <div style="font-size:12px;font-weight:600;color:var(--g5);margin-bottom:.3rem">
        ${notfound.length} student(s) not found in portal (searched ${mcYearLabel}) â€” type a matricule to assign</div>
      <div style="display:flex;flex-direction:column;gap:4px">`;
    for(const r of notfound) html += manualRow(r);
    html += `</div></div>`;
  }
  ...
  if(skipped.length){
    html += `<div style="margin-top:.5rem">
      <div style="font-size:12px;font-weight:600;color:var(--g5);margin-bottom:.3rem">
        ${skipped.length} student(s) skipped (no name) â€” assign a matricule if known</div>
      <div style="display:flex;flex-direction:column;gap:4px">`;
    for(const r of skipped) if(r.matricule || r.name) html += manualRow(r);
    html += `</div></div>`;
  }
    ...
    <div class="card-body" style="padding:.75rem 1rem">
      <div class="flex aic jbs" style="margin-bottom:.4rem">
        <div class="flex aic" style="gap:8px">
          <input type="checkbox" style="accent-color:var(--blue);width:15px;height:15px"
            onchange="mcSelToggle('${esc(r.matricule)}','${esc(r.api_matricule||'')}','${esc(r.name)}',this.checked)"
            ${mcSel.has(r.matricule)?'checked':''} ${r.api_matricule?'':'disabled'}>
