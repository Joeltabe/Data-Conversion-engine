...
function mcSelToggle(oldMat, newMat, on){
  if(on && newMat) mcSel.set(oldMat, newMat);
  else mcSel.delete(oldMat);
  renderMatriculeResults(mcResults);
}
...
function setMcSelAll(which){
  for(const r of (mcResults || [])){
    ...
    if(ok && r.api_matricule) mcSel.set(r.matricule, r.api_matricule);
    else mcSel.delete(r.matricule);
  }
  renderMatriculeResults(mcResults);
}

function mcBulkBar(mismatched, review){
  let sel = 0;
  for(const r of [...mismatched, ...review]) if(mcSel.has(r.matricule)) sel++;
  return `<div class="flex gap-2 aic" style="margin:0 0 .75rem;padding:.5rem .75rem;background:var(--g1);border:1px solid var(--g2);border-radius:8px;flex-wrap:wrap">
    ...
    <button class="btn btn-outline btn-sm" onclick="setMcSelAll('all')">Select all</button>
    ${sel?`<button class="btn btn-primary btn-sm" onclick="bulkMigrate()">ðŸšš Migrate ${sel} â†’ portal</button>`:''}
    ${sel?`<button class="btn btn-outline btn-sm" onclick="setMcSelAll('none')">Clear</button>`:''}
    <span class="text-sm muted">${sel} selected</span>
  </div>`;
}

async function bulkMigrate(){
  if(!jobId || !mcSel.size) return;
  const n = mcSel.size;
  if(!confirm(`Migrate ${n} student(s) to their portal matricule?\n\nThis updates their matricule for the Excel export.`)) return;
  const mappings = [...mcSel.entries()].map(([old_m, new_m]) => ({old_matricule: old_m, new_matricule: new_m}));
  try{
    ...
    for(const [old_m] of [...mcSel]){
      const x = mcResults.find(v => v.matricule === old_m);
      if(x && x.api_matricule){
        x.matricule = x.api_matricule; x.status = 'verified'; x.mismatch = false;
      }
      mcSel.delete(old_m);
    }
    await loadStudents();
...
function renderMatriculeResults(results){
  ...
  if(mismatched.length || review.length) html += mcBulkBar(mismatched, review);
      ...
      <div class="flex aic jbs" style="margin-bottom:.4rem">
        <div class="flex aic" style="gap:8px">
          <input type="checkbox" style="accent-color:var(--blue);width:15px;height:15px"
            onchange="mcSelToggle('${esc(r.matricule)}','${esc(r.api_matricule||'')}',this.checked)"
